...
from source.models.cg_model import CGModel
# from source.models.old_cg_model import CGOldModel
from source.utils.diffusion_utils import get_timestep_embedding
...
def get_obrmsd(mol1_path, mol2_path, cache_name=None):
    ...
    with warnings.catch_warnings():
        ...
        return_code = subprocess.run(f"obrms {mol1_path} {mol2_path} > .openbabel_cache/obrmsd_{cache_name}.rmsd",
                                     shell=True)
        print(return_code)
    obrmsd_output = read_strings_from_txt(f".openbabel_cache/obrmsd_{cache_name}.rmsd")
    rmsds = [line.split(" ")[-1] for line in obrmsd_output]
...
def get_optimizer_and_scheduler(args, model, scheduler_mode='min', step=0, optimizer=None):
    ...
    scheduler_plateau = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode=scheduler_mode, factor=0.7,
