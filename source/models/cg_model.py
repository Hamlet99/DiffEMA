...
from source.models.layers import GaussianSmearing, AtomEncoder
from source.models.tensor_layers import TensorProductConvLayer, get_irrep_seq
from source.utils import so3, torus
from source.datasets.process_mols import lig_feature_dims, rec_residue_feature_dims
...
class CGModel(torch.nn.Module):
    ...
    def embedding(self, data):
        if not hasattr(data['patch_ed'], "rec_node_attr"):
            ...
            for l in range(len(self.rec_emb_layers)):
                edge_attr_ = torch.cat([rec_edge_attr, rec_node_attr[data['patch_ed', 'patch_ed'].edge_index[0], :self.ns], rec_node_attr[data['patch_ed', 'patch_ed'].edge_index[1], :self.ns]], -1)
                rec_node_attr = self.rec_emb_layers[l](rec_node_attr, data['patch_ed', 'patch_ed'].edge_index, edge_attr_, rec_edge_sh, edge_weight=rec_edge_weight)

            data['patch_ed'].rec_node_attr = rec_node_attr
            data['patch_ed', 'patch_ed'].rec_edge_attr = rec_edge_attr
            data['patch_ed', 'patch_ed'].edge_sh = rec_edge_sh
            data['patch_ed', 'patch_ed'].edge_weight = rec_edge_weight

        # receptor embedding
        rec_sigma_emb = self.rec_sigma_embedding(self.timestep_emb_func(data.complex_t['tr']))
        rec_node_attr = data['patch_ed'].rec_node_attr + 0
        rec_node_attr[:, :self.ns] = rec_node_attr[:, :self.ns] + rec_sigma_emb[data['patch_ed'].batch]
        rec_edge_attr = data['patch_ed', 'patch_ed'].rec_edge_attr + rec_sigma_emb[data['patch_ed'].batch[data['patch_ed', 'patch_ed'].edge_index[0]]]

        lig_node_attr, lig_edge_index, lig_edge_attr, lig_edge_sh, lig_edge_weight = self.ligand_embedding(data)

        return lig_node_attr, lig_edge_index, lig_edge_attr, lig_edge_sh, lig_edge_weight, \
               rec_node_attr, data['patch_ed', 'patch_ed'].edge_index, rec_edge_attr, data['patch_ed', 'patch_ed'].edge_sh, data['patch_ed', 'patch_ed'].edge_weight
    ...
    def build_rec_conv_graph(self, data):
        ...
        node_attr = data['patch_ed'].x

        # this assumes the edges were already created in preprocessing since protein's structure is fixed
        edge_index = data['patch_ed', 'patch_ed'].edge_index
        src, dst = edge_index
        edge_vec = data['patch_ed'].pos[dst.long()] - data['patch_ed'].pos[src.long()]
    ...
    def build_cross_conv_graph(self, data, cross_distance_cutoff):
        ...
        if torch.is_tensor(cross_distance_cutoff):
            ...
            edge_index = radius(data['patch_ed'].pos / cross_distance_cutoff[data['patch_ed'].batch],
                                data['ligand'].pos / cross_distance_cutoff[data['ligand'].batch], 1,
                                data['patch_ed'].batch, data['ligand'].batch, max_num_neighbors=10000)
        else:
            edge_index = radius(data['patch_ed'].pos, data['ligand'].pos, cross_distance_cutoff,
                                data['patch_ed'].batch, data['ligand'].batch, max_num_neighbors=10000)

        src, dst = edge_index
        edge_vec = data['patch_ed'].pos[dst.long()] - data['ligand'].pos[src.long()]
