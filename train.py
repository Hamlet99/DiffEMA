...
from source.parser import parse_train_args
from source.datasets.loader import construct_loader
from source.utils.diffusion_utils import t_to_sigma as t_to_sigma_compl
from source.utils.training_utils import train_epoch, test_epoch, loss_function, inference_epoch_fix
from source.utils.general_utils import save_yaml_file, get_optimizer_and_scheduler, get_model, ExponentialMovingAverage
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.scheduler == 'layer_linear_warmup' and (epoch + 1) % args.warmup_dur == 0:
            ...
            if step < args.num_conv_layers + 2:
                ...
                optimizer, scheduler = get_optimizer_and_scheduler(args, model, step=step,
        ...
        logs = {}
        train_losses = train_epoch(model, train_loader, optimizer, device, t_to_sigma, loss_fn,
        ...
        if epoch > freeze_params:
            ...
            if args.use_ema: ema_weights.copy_to(
                model.parameters())  # load ema parameters into model for running validation and inference
        ...
        if args.val_inference_freq != None and (epoch + 1) % args.val_inference_freq == 0:
            inf_dataset = [val_loader.dataset.get(i) for i in
                           range(min(args.num_inference_complexes, val_loader.dataset.__len__()))]
            ...
            print("Epoch {}: Val inference rmsds_lt2 {:.3f} rmsds_lt5 {:.3f} min_rmsds_lt2 {:.3f} min_rmsds_lt5 {:.3f}"
                  .format(epoch, inf_metrics['rmsds_lt2'], inf_metrics['rmsds_lt5'], inf_metrics['min_rmsds_lt2'],
        ...
        if args.train_inference_freq != None and (epoch + 1) % args.train_inference_freq == 0:
            inf_dataset = [train_loader.dataset.get(i) for i in
                           range(min(min(args.num_inference_complexes, 300), train_loader.dataset.__len__()))]
            inf_metrics = inference_epoch_fix(model, inf_dataset, device, t_to_sigma, args)
            print(
                "Epoch {}: Train inference rmsds_lt2 {:.3f} rmsds_lt5 {:.3f} min_rmsds_lt2 {:.3f} min_rmsds_lt5 {:.3f}"
                .format(epoch, inf_metrics['rmsds_lt2'], inf_metrics['rmsds_lt5'], inf_metrics['min_rmsds_lt2'],
                        inf_metrics['min_rmsds_lt5']))
        ...
        if args.inference_secondary_metric is not None and args.inference_secondary_metric in logs.keys() and \
                (args.inference_earlystop_goal == 'min' and logs[
                    args.inference_secondary_metric] <= best_val_secondary_value or
                 args.inference_earlystop_goal == 'max' and logs[
                     args.inference_secondary_metric] >= best_val_secondary_value):
...
def main_function():
    ...
    if args.cudnn_benchmark:
        torch.backends.cudnn.benchmark = True  # check is needed or not
