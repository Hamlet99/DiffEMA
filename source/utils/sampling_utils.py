...
from source.utils.diffusion_utils import modify_conformer, set_time, modify_conformer_batch
from source.utils.torsion import modify_conformer_torsion_angles
from scipy.spatial.transform import Rotation as R

from source.utils.general_utils import crop_beyond
...
def randomize_position(data_list, no_torsion, no_random, tr_sigma_max, pocket_knowledge=False, pocket_cutoff=7,
    ...
    center_pocket = data_list[0]['patch_ed'].pos.mean(dim=0)
    if pocket_knowledge:
        complex = data_list[0]
        d = torch.cdist(complex['patch_ed'].pos, torch.from_numpy(complex['patch_ed'].orig_pos[0]).float() - complex.original_center)
        label = torch.any(d < pocket_cutoff, dim=1)

        if torch.any(label):
            center_pocket = complex['patch_ed'].pos[label].mean(dim=0)
        else:
            print("No pocket residue below minimum distance ", pocket_cutoff, "taking closest at", torch.min(d))
            center_pocket = complex['patch_ed'].pos[torch.argmin(torch.min(d, dim=1)[0])]
    ...
    for complex_graph in data_list:
        ...
        if not no_random:  # note for now the torsion angles are still randomised
            if choose_residue:
                idx = random.randint(0, len(complex_graph['patch_ed'].pos)-1)
                tr_update = torch.normal(mean=complex_graph['patch_ed'].pos[idx:idx+1], std=0.01)
            elif initial_noise_std_proportion >= 0.0:
                std_rec = torch.sqrt(torch.mean(torch.sum(complex_graph['patch_ed'].pos ** 2, dim=1)))
