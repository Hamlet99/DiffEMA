...
from source.confidence.dataset import ListDataset
from source.utils import so3, torus
from source.utils.molecules_utils import get_symmetry_rmsd
from source.utils.sampling_utils import randomize_position, sampling
...
def loss_function(tr_pred, rot_pred, tor_pred, sidechain_pred, data, t_to_sigma, device, tr_weight=1, rot_weight=1,
    ...
    if not no_torsion:
        ...
        if apply_mean:
            tor_loss, tor_base_loss = tor_loss.mean() * torch.ones(1,
                                                                   dtype=torch.float), tor_base_loss.mean() * torch.ones(
                1, dtype=torch.float)
        else:
        ...
        if apply_mean:
            ...
            tor_loss, tor_base_loss = torch.zeros(len(rot_loss), dtype=torch.float), torch.zeros(len(rot_loss),
                                                                                                 dtype=torch.float)
    ...
    if apply_mean:
        ...
        backbone_loss, backbone_base_loss = torch.zeros(len(rot_loss), dtype=torch.float), torch.zeros(
...
def train_epoch(model, loader, optimizer, device, t_to_sigma, loss_fn, ema_weights):
    ...
    meter = AverageMeter(['loss', 'tr_loss', 'rot_loss', 'tor_loss', 'backbone_loss', 'sidechain_loss',
                          'tr_base_loss', 'rot_base_loss', 'tor_base_loss', 'backbone_base_loss',
                          'sidechain_base_loss'])
        ...
        try:
            ...
            if test_sigma_intervals > 0:
                complex_t_tr, complex_t_rot, complex_t_tor = [
                    torch.cat([data[i].complex_t[noise_type] for i in range(len(data))]) for
                ...
                meter_all.add([loss_tuple[0].cpu().detach(), *loss_tuple[1:]],
                              [sigma_index_tr, sigma_index_tr, sigma_index_rot, sigma_index_tor, sigma_index_tr,
                               sigma_index_tr,
...
def inference_epoch_fix(model, complex_graphs, device, t_to_sigma, args):
    ...
    for orig_complex_graph in tqdm(loader):
        ...
        while predictions_list == None:
            try:
                predictions_list, confidences = sampling(data_list=data_list,
        ...
        try:
            orig_ligand_pos = orig_complex_graph['ligand'].orig_pos[:,
                              filterHs] - orig_complex_graph.original_center.cpu().numpy()
        except Exception as e:
